...
봇을 실행하기 위해 Ubuntu 서버를 설치한 후, 원격으로 접속하여 관리하려면 SSH 설정이 필요합니다.

#### 1. Ubuntu에 SSH 서버 설치

Ubuntu 서버 터미널에서 다음 명령어를 입력하여 OpenSSH 서버를 설치합니다.
...
# SSH 서비스 상태 확인 (active (running) 상태인지 확인)
sudo systemctl status ssh
```

#### 2. VS Code로 원격 접속 (Remote - SSH)

로컬 컴퓨터(Windows/Mac)에서 VS Code를 사용하여 서버에 편리하게 접속할 수 있습니다.

1. **확장 프로그램 설치**:
    VS Code에서 **Remote - SSH** 확장 프로그램을 설치합니다.
    [👉 VS Code Remote - SSH 설치 링크](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh)

2. **서버 접속**:
    * VS Code 왼쪽 하단의 녹색 아이콘(><) 클릭 > **Connect to Host...** 선택
    * `username@your_server_ip` 형식으로 입력 (예: `ser1@192.168.0.10`)
    * 비밀번호 입력 후 접속 완료
...
### 8. 시스템 부팅 시 자동 실행 (Linux Systemd)
