...
def main():
    ...
    # Initialize Exchange
    exchange_config = {
        'apiKey': (os.getenv('BYBIT_API_KEY_15M') or os.getenv('BYBIT_API_KEY') or config['exchange']['api_key']).strip(),
        'secret': (os.getenv('BYBIT_API_SECRET_15M') or os.getenv('BYBIT_API_SECRET') or config['exchange']['api_secret']).strip(),
